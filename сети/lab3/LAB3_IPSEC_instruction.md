...
- **PSK (IKE):** `<Фамилия>67` — подставьте свою фамилию **без пробела** перед `67`; на обоих ASA должен быть **одинаковый** ключ.
- **Имена устройств:** включайте вариант **19**, например: `DSL Modem_19_1`, `ASA-CAMPUS-VPN_19`, `ASA-BRANCH01_19`.
- **DNS A-запись:** `www.<Фамилия_ИО>.iu5` → IP веб-сервера в вашей DMZ/серверном сегменте (например, из `172.67.115.0/21`).

---

## 4. Конфигурация: головной ASA (корпоративная сеть)

Замените `<Фамилия>67` на свой PSK. При необходимости скорректируйте под Packet Tracer/стенд (имена интерфейсов, доп. NAT).
...
tunnel-group 134.67.19.18 ipsec-attributes
 ikev1 pre-shared-key <Фамилия>67
!
```
...
2. DNS: **A-запись** `www.<Фамилия_ИО>.iu5` на IP сервера.
